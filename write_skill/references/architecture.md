...
- `call_tools` 节点（single）优先执行：将 `skill_system_prompt`（含文档地图）**与对话历史**注入给 LLM（顺序 system(skill)→history→query，与 extract/generate 一致），LLM 按需调用内置工具（如 `load_policy_file`），结果写入 `tool_results`。**带 history 是为追问场景**——否则工具决策看不到上下文，"把刚才查到的生成 Word""读上面那个文件"无法解析指代。**失败也写 `tool_results`**：工具找不到 / 执行抛错时追加 `[工具调用失败]`/`[工具执行失败]` 标记（同时记入 `tool_items`），否则 `tool_results` 为空、`generate` 的 `memory` 段看不到失败，模型会误以为工具成功或编造结果
...
- `retrieve` 节点内部经 `rag_fn`（`api/agent.py` 闭包）调 `HybridRetriever.search()`（`agent_service/rag/simple_rag.py`）：用 `ThreadPoolExecutor` 并行执行 BM25 检索与向量检索（embedding API 调用 + Chroma 查询），再做混合分数融合。
  ...
  - **检索旋钮统一兜底 config，请求体可覆盖**：两条链路（`/agent/chat`、`/query`）的 `top_k`/`bm25_k`/`vector_k`/`bm25_weight`/`chunk_size`/`chunk_overlap`/`separators` 缺省时都取 `services.load_config()`（`config.yaml`），请求体显式提供则覆盖（判定「key 在且非 None」，保留 `bm25_weight=0.0` 等合法假值）。**两个历史不一致已修复**：① `/agent/chat` 的 `top_k` 曾写死兜底 `5`，使 config 的 `top_k` 在对话链路失效；② `/query` 的 `chunk_size` 曾写死兜底 `400`，与对话链路用的 `cfg.chunk_size`（如 800）不同 → `get_rag` 缓存键含 `chunk_size`，管理员测试按 400 另建一套索引、召回与线上对话不一致。`/agent/chat` 仍只暴露 `top_k` 给请求体覆盖，`bm25_k`/`vector_k`/`bm25_weight` 只读 config。
...
**文档文本提取**：`extract_text_from_file(path)` 是 `read_document` 工具、`/ingest` 清洗、**以及 RAG 索引**（`DocumentLoader.load` 对 `.pdf/.docx`）的单一实现：
`.pdf`→PyMuPDF + pdfplumber、`.docx`→unstructured（回退 python-docx）、纯文本→`text_utils.read_text_smart` 自动识别编码；`.doc` 与缺依赖时抛错。
**RAG 索引归口**：`DocumentLoader.load` 对 `.pdf/.docx` 走 `extract_text_from_file`（延迟导入避免循环依赖），其余走 `read_text_smart`；单文件失败只跳过该文件、不挂整次重建。`allowed_extensions` 现含 `.pdf/.docx`（`RAGConfig` 默认与 `config.yaml` 同步），故二进制文档**已能进入混合检索**。`/ingest` 不再自行 embed/写 Chroma（会被整集合重建覆盖），嵌入/索引统一由 `services._rebuild_rag` 负责。
