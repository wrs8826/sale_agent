...
| `assets/marked.min.js` / `assets/purify.min.js` | **本地托管**的 marked + DOMPurify（不走外网 CDN）。`user.html` 的 `renderMd` 必经 `DOMPurify.sanitize(marked.parse(...))` 消毒后再 `innerHTML`，防存储型 XSS（回答/用户输入/工具结果均注入 DOM）；marked/DOMPurify 缺失或解析异常时降级为纯文本（`_plainFallback`）。复制按钮 `copyMsg` 在 LAN HTTP（非安全上下文，`navigator.clipboard` 不可用）下退 `execCommand('copy')`。改前端渲染**勿绕过 `renderMd` 直接拼 HTML**（见 common-pitfalls #42）|
