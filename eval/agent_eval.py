...
def _judge_faithfulness(client, model, query, context, answer) -> Optional[Dict]:
    # 上下文预算放大到 12000：top_k=10 个块（chunk_size 可达 800）就近 8000 字符，
    # 旧的 4000 会把拼在其后的工具结果（政策原文 / 当前时间 / 文档内容）整段截掉，
    # 导致 policy/tool 类问题的真实证据看不到、被误判为幻觉。
    ctx = (context or "（无检索上下文）")[:12000]
...
def main() -> None:
    ...
    C = {"by_cat": {}}  # 置信度：按 category 分组 {cat: {"scores": [...], "pass": int}}
    ...
    for i, item in enumerate(items):
        ...
        # 按 category 单独累计：rag 才是"该过阈"的锚，ood 是"该低"的锚；
        # policy/tool 不在检索索引内（走 load_policy_file / 工具），检索分天然偏低，
        # 单列为信息项，不并入"域内"以免稀释过阈率。
        g = C["by_cat"].setdefault(cat or "(未分类)", {"scores": [], "pass": 0})
        g["scores"].append(top_score)
        ...
        if run_agent:
            ...
            if judge_client is not None and cat != "ood":
                # 工具结果是 agent 实际依据（政策原文 / 时间 / 文档内容），优先放前面；
                # 检索块作为补充。否则 policy 类的真实证据会被检索噪声挤到上下文预算之外。
                ctx_parts = [tr["result"] for tr in run["tool_results"] if tr.get("result")]
    ...
    _CONF_NOTE = {
        "rag": "应高且过阈（答案在检索索引内）",
        "policy": "走 load_policy_file，不在检索索引，分偏低正常",
        "tool": "靠工具非检索，分偏低正常",
        "ood": "应低于阈值（库内无答案，正确回退）",
    }
    _CONF_ORDER = ["rag", "policy", "tool", "ood"]
    by_cat = C["by_cat"]
    cats = [c for c in _CONF_ORDER if c in by_cat] + [c for c in by_cat if c not in _CONF_ORDER]
    for c in cats:
        g = by_cat[c]
        if not g["scores"]:
            continue
        mean = statistics.mean(g["scores"])
        rate = pct(g["pass"], len(g["scores"]))
        print(f"  {c:<8} 平均={mean:.3f}  过阈率={rate:5.1%}  ({len(g['scores'])} 条)  {_CONF_NOTE.get(c, '')}")
    # 分离度只用 rag（该高）对 ood（该低）算，最能反映门控判别力
    if by_cat.get("rag", {}).get("scores") and by_cat.get("ood", {}).get("scores"):
        sep = statistics.mean(by_cat["rag"]["scores"]) - statistics.mean(by_cat["ood"]["scores"])
        print(f"  → rag/ood 分离度 = {sep:+.3f}（越大门控判别力越强）")
        ...
        payload = {
            ...
            "confidence": {
                "by_category": {
                    c: {
                        "mean": statistics.mean(g["scores"]),
                        "pass_rate": pct(g["pass"], len(g["scores"])),
                        "n": len(g["scores"]),
                    }
                    for c, g in C["by_cat"].items() if g["scores"]
                },
                "rag_ood_separation": (
                    statistics.mean(C["by_cat"]["rag"]["scores"]) - statistics.mean(C["by_cat"]["ood"]["scores"])
                    if C["by_cat"].get("rag", {}).get("scores") and C["by_cat"].get("ood", {}).get("scores")
                    else None
                ),
            },
            "tool": {
